 ...
 * PENDING → APPROVED → ISSUED, side branch REJECTED. amount stored as TEXT
 * (cents math in core). Evidence/contract/invoice files are Document ids. */
export const invoiceRequests = sqliteTable(
  "InvoiceRequest",
  {
    id: text("id").primaryKey(),
    matterId: text("matter_id"),
    noMatterReason: text("no_matter_reason"),
    amount: text("amount").notNull(),
    title: text("title"),
    ...
    // LAWYER_FEE | CONSULTING_FEE | AGENCY_FEE | OTHER
    ...
    evidenceDocIdsJson: text("evidence_doc_ids_json").notNull().default("[]"),
    invoiceNo: text("invoice_no"),
    ...
    contractScanId: text("contract_scan_id").unique(),
    invoiceFileId: text("invoice_file_id").unique(),
    createdAt: integer("created_at", { mode: "timestamp" }).notNull(),
    updatedAt: integer("updated_at", { mode: "timestamp" }).notNull(),
  },
  (t) => [
    index("InvoiceRequest_matter_idx").on(t.matterId, t.status),
    index("InvoiceRequest_status_idx").on(t.status, t.requestedAt),
    index("InvoiceRequest_requester_idx").on(t.requestedById),
  ],
);

/** In-app notification (通知中心). Per-user feed, generated by status changes
 * (seal/archive), task assignment, SMS arrival, and the reminder scans. */
export const notifications = sqliteTable(
  "Notification",
  {
    id: text("id").primaryKey(),
    userId: text("user_id").notNull(),
    // PRESERVATION_EXPIRY | HEARING_REMINDER | DEADLINE_REMINDER |
    // SEAL_STATUS_CHANGE | SMS_ARRIVAL | TASK_ASSIGNED | ARCHIVE_* | SYSTEM
    type: text("type").notNull(),
